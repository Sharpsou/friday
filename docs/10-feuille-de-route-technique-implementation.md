...
État d’exécution au 8 août 2026, sur le candidat construit après `22dc523` : les points 1 et 2 sont implémentés avec la même voie locale/outbox, y compris heure et durée facultatives, migration SQLite N−1 et affichages `Liste`/`Semaine`/`Mois` dans `Maison > Agenda`. La partie responsable du point 3 est également implémentée avec deux identifiants pilotes stables, un filtre commun aux trois vues et des noms configurables localement. Une roue dentée donne aussi accès au choix local de quatre palettes sombres. Les profils réels restent réservés à l’authentification/appairage. `pnpm verify` réussit avec 27 tests unitaires/intégration et 9 scénarios Chrome mobile. Terminer/rouvrir et date/agenda sont validés physiquement sur l’A17. Les recettes courtes [`recipes/galaxy-a17-lot-1a-assignee.md`](recipes/galaxy-a17-lot-1a-assignee.md) et [`recipes/galaxy-a17-lot-1a-settings.md`](recipes/galaxy-a17-lot-1a-settings.md) restent les checkpoints immédiats ; après validation, la prochaine cible du point 3 est récurrence simple et note.
