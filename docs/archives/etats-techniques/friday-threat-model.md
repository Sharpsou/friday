...
> Archive conservée lors de la refonte du 6 septembre 2026, depuis `docs/friday-threat-model.md` au commit `894a50d`. Les états, chiffres et commandes ci-dessous décrivent leurs dates ; ils ne pilotent pas une reprise. Consulter la [référence active](../../friday-threat-model.md).

## Ajout Veille orchestrée — TM-015

Une source peut tenter de polluer la mémoire thématique par des concepts ou des
sujets trompeurs. Le préfiltrage local, les sorties Qwen bornées et validées, la
provenance obligatoire par article, les concepts suggérés secondaires par défaut,
la confirmation humaine des concepts suivis et l'isolation par profil limitent ce
risque. Une information plausible mais fausse reste un risque résiduel : les
sources justificatives demeurent donc visibles sous chaque sujet.
...
- Chat : conversations privées par profil, file persistante, Gemma 4 ou Qwen 3.5 local et recherche Tavily optionnelle avec quotas.
...
| Veilles, flux et digests privés      | Révèlent les intérêts et lectures d’un profil   | C/I/A                      |
...
| Collecteur Veille           | HTTPS sortant planifié  | hub → flux/page publique → Ollama  | SSRF, redirections, XML/HTML hostile, injection et épuisement | `apps/hub/src/watch/`                                         |
...
15. Un flux validé redirige vers une adresse privée ou publie une instruction hostile → tentative de SSRF ou d’injection indirecte dans Qwen.
...
| TM-014    | source RSS/page publique compromise      | veille active et contenu récupéré                | redirige vers le LAN ou injecte des instructions      | lecture réseau interne ou digest trompeur | réseau local, veille, modèle          | HTTPS public, DNS/IP vérifiés à chaque redirection, limites, texte seul, prompt isolé, JSON validé, aucun outil | DNS rebinding reste une classe à surveiller ; robots.txt simplifié   | journal des refus URL, erreurs de sortie structurée et volume par source                                         | medium                                                         | high       | high            |
...
| `apps/hub/src/watch/`                                    | fetch sortant, SSRF, XML/HTML et analyse LLM   | TM-014                 |
...
- [x] surface Veille, SSRF et injection indirecte documentées ;
