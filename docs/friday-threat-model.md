# Friday — modèle de menace Lot 1A et budget partagé

Date : 9 août 2026
Révision : budget partagé implémenté, seed réel et recette physique en attente

## Executive summary

Friday concentre des données familiales sur un hub Windows accessible depuis le LAN et conserve une copie chiffrée dans une PWA. Le candidat du Lot 1A lie maintenant chaque session à un adulte, un foyer et un appareil révocable, ferme l'inscription publique, borne les codes d'appairage et protège push/pull. Le risque résiduel principal est l'accès à un cache déjà téléchargé sur un téléphone perdu ou révoqué : la révocation serveur ne peut pas l'effacer à distance.

## Scope and assumptions

Périmètre : `apps/web`, `apps/hub`, `packages/contracts`, `packages/domain`, la base SQLite, IndexedDB, les scripts d'exploitation et la frontière d'authentification/appairage implémentée.

Hors périmètre : Google Calendar, Drive, RSS, Ollama, accès Internet au hub et projets sources externes. La transformation ponctuelle des classeurs budget est couverte comme une opération administrateur ; les classeurs sources restent externes et en lecture seule.
...
- PWA → IndexedDB : tâches, budget, opérations et identité d'appareil ; AES-GCM avec AAD propre à chaque type d'entité et transactions Dexie atomiques.
...
Contrôles du candidat : inscription publique masquée, bootstrap fermé après le premier membre, cookie durci, mutations d'authentification limitées aux origines approuvées, code d'appairage à usage unique et expirant, session liée à l'appareil, identité de push vérifiée et sessions supprimées à la révocation. Les parcours sont couverts par intégration et Chrome mobile ; la recette réelle reste exigée.
