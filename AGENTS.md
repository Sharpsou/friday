...
- Le prototype zéro AlphaBot2-Pi est réinstallé sous Raspberry Pi OS Trixie 32 bits et intégré à l’onglet Robot. Caméra CSI, capteurs passifs, roues et servos réels sont téléopérables ; la production n’expose plus de mode simulation. Le servo pan tremble par intermittence, des sous-tensions ont été enregistrées et aucune autonomie n’est autorisée. Pour tout travail Robot, lire d’abord `docs/22-checkpoint-robot-alphabot2-2026-08-24.md`, puis `docs/runbooks/robot-alphabot2.md` ; le document 21 n’est que le journal chronologique.
