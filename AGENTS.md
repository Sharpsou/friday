...
## État du workspace au 18 août 2026
...
`pnpm verify` réussit sur le candidat du 18 août 2026 avec 192 tests unitaires/intégration et 23 scénarios Chrome mobile. Une sauvegarde pré-migration est conservée hors dépôt sous `D:\FridayData\backups`. SQLite est à la migration 19 : 14–15 portent l’orchestrateur Chat et le modèle, 16–18 la Veille orchestrée et 19 les fournisseurs/diagnostics Exa. Dexie est à la version 7. Le candidat doit être reconstruit et redémarré sur `https://192.168.1.14:8443` après toute évolution du runtime.

Le propriétaire a initialisé le foyer le 9 août 2026. L'appairage d'un second appareil n'est pas validé : le RG405M sous Firefox 151.0.3 atteint Friday mais conserve un avertissement de certificat. Sur l’iPhone de la compagne, la réception d’une mise à jour PWA a été confirmée le 9 août et la suppression de l’auto-zoom des champs Tâche/Course a été confirmée le 18 août dans Chrome iOS, sans bloquer le zoom manuel. L’appairage, l’authentification et le parcours offline/convergence iPhone restent ouverts ; ne pas étendre ces preuves au-delà des scénarios réellement joués.
...
2. poursuivre `docs/recipes/iphone-pwa-update.md`, puis l'appairage et la recette offline sur l'iPhone ;
