...
const HIGH_RISK =
  /\b(m[ée]dicament|sympt[oô]me|diagnostic|juridique|avocat|loi|fiscal|imp[oô]t|investir|placement|cr[ée]dit)\b/iu;
const CURRENT =
...
export function routeDeterministically(question: string): RouteDecision | null {
  if (SOURCE_REQUIRED.test(question)) {
    return codeDecision('web', 'source_required', [question]);
  }
  if (HIGH_RISK.test(question)) {
    return codeDecision('web', 'high_risk', [question]);
  }
  if (CURRENT.test(question)) {
    return codeDecision('web', 'current_information', [question]);
  }
  if (RECOMMENDATION.test(question)) {
    return codeDecision('web', 'recommendation', [question]);
  }
  if (WRITING.test(question)) {
    return codeDecision('local', 'writing_or_conversation');
  }
  if (STABLE_EXPLANATION.test(question)) {
    return codeDecision('local', 'stable_explanation');
  }
  return null;
}
...
export function acceptClassifierRoute(input: unknown): RouteDecision {
  ...
  if (parsed.route === 'local' && parsed.queries.length > 0) {
    throw new Error('LOCAL_ROUTE_MUST_NOT_QUERY');
  }
  if (parsed.route === 'web' && parsed.queries.length === 0) {
...
export async function routeQuestion(
  ...
  return acceptClassifierRoute(payload);
}
